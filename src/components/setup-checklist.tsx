...
export function SetupChecklist() {
  ...
  const completed = steps.filter((s) => data[s.id].complete).length;
  const progress = Math.round((completed / steps.length) * 100);
  const next = steps.find((s) => !data[s.id].complete);
      ...
      <CardHeader className="pb-3">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-muted-foreground">We’ll guide you through the essentials. SchoolFlow tracks your progress automatically.</p>
          </div>
      ...
      <CardContent className="space-y-3">
        <div className="rounded-lg border bg-card p-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="rounded-lg bg-primary/10 p-2.5 text-primary"><next.icon className="h-5 w-5" /></div>
            <div className="min-w-0 flex-1"><p className="text-xs font-medium uppercase tracking-wide text-primary">Recommended next</p><p className="mt-0.5 font-semibold">{actionFor(next, nextStatus.count)}</p><p className="mt-0.5 text-sm text-muted-foreground">{nextStatus.detail || next.description}</p></div>
            <Button size="sm" asChild><Link to={next.to}>Continue setup<ArrowRight className="ml-1.5 h-3.5 w-3.5" /></Link></Button>
          </div>
        </div>
        <div className="space-y-1.5">
          {steps.map((step, index) => { const status = data[step.id]; const done = status.complete; const Icon = step.icon; const isNext = step.id === next.id; return (
        ...
        <p className="pt-1 text-center text-[11px] text-muted-foreground">You can complete these steps in any order. Your progress is saved automatically.</p>
      </CardContent>
    </Card>
  );
}
