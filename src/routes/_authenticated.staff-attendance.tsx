...
import { listStaffAttendance, upsertStaffAttendance, type StaffAttStatus } from "@/lib/staff-attendance.functions";
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-3 sm:px-6 py-6 space-y-5">
      ...
      {q.isLoading ? (
        ...
        <Card><CardContent className="p-8 text-center text-sm text-muted-foreground">No active staff.</CardContent></Card>
      ) : (
              ...
              <Card key={s.id}>
                <CardContent className="flex flex-wrap items-center gap-3 p-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">{s.position}{s.matricule ? ` · ${s.matricule}` : ""}</div>
                  </div>
