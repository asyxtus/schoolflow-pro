...
const summaryQO = queryOptions({
  ...
  queryFn: () => librarySummary(),
});
const booksQO = queryOptions({
  queryKey: ["library", "books"],
  queryFn: () => listBooks({ data: {} }),
});
const loansQO = queryOptions({
  queryKey: ["library", "loans"],
  queryFn: () => listLoans({ data: { status: "all" } }),
});
    ...
    try {
      await save({
        data: {
          ...
          initial_copies: form.id ? undefined : form.initial_copies,
        },
      });
      toast.success(
        form.id
          ? "Saved"
          : `Saved · ${form.initial_copies} cop${form.initial_copies === 1 ? "y" : "ies"} added`,
      );
      setOpen(false);
      router.invalidate();
    } catch (e) {
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{form.id ? "Edit" : "Add"} book</DialogTitle>
              </DialogHeader>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setForm({
                          ...form,
                          initial_copies: Math.max(0, Number(e.target.value || 0)),
                        })
                      }
                    />
                  </div>
                ...
                {filtered.map((b) => (
                  <tr key={b.id} className="border-t border-border">
                    ...
                    <td className="text-right">
                      <Button size="icon" variant="ghost" onClick={() => setCopiesOpen(b.id)}>
                      ...
                      <Button
                        ...
                        variant="ghost"
                        onClick={async () => {
                          if (confirm("Delete book and all copies?")) {
...
function CopiesDialog({ bookId, onClose }: { bookId: string; onClose: () => void }) {
  const router = useRouter();
  const copiesQO = queryOptions({
    queryKey: ["library", "copies", bookId],
    queryFn: () => listCopies({ data: { bookId } }),
  });
  ...
  async function doAdd() {
    try {
      await add({ data: { book_id: bookId, count, barcode_prefix: prefix || undefined } });
      toast.success("Added");
      router.invalidate();
    } catch (e) {
      toast.error((e as Error).message);
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Copies</DialogTitle>
        </DialogHeader>
        ...
        <div className="mt-2 max-h-72 overflow-y-auto rounded-md border border-border">
          {copies.length === 0 ? (
            <p className="p-3 text-sm text-muted-foreground">No copies yet.</p>
          ) : (
            <table className="w-full text-sm">
              <tbody>
                {copies.map((c) => (
            ...
            <div className="space-y-3">
              ...
              {form.book_id && (
                <div>
                  ...
                  <Select
                    value={form.copy_id}
                    onValueChange={(v) => setForm({ ...form, copy_id: v })}
                  >
                    <SelectTrigger>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setForm({
                      ...form,
                      borrower_type: v as BorrowerType,
                      student_id: "",
                      staff_id: "",
                      student_name: "",
                    })
                      ...
                      <Input
                        ...
                        onChange={async (e) => {
                          ...
                          if (e.target.value.length >= 2)
                            setResults(await search({ data: { q: e.target.value } }));
                          else setResults([]);
                        }}
                      />
                      {results.length > 0 && (
                        <div className="mt-2 max-h-32 overflow-y-auto rounded-md border border-border">
                          {results.map((r) => (
                            ...
                            >
                              ...
                              <span className="text-muted-foreground">{r.matricule}</span>
                            </button>
                          ))}
                        </div>
                      )}
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              if (confirm("Mark as lost?")) {
                                await ret({ data: { id: l.id, lost: true } });
                                router.invalidate();
                              }
                            }}
                          >
      ...
      <CardContent>
        {res.length === 0 ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {res.map((r) => {
                const b = (r as { library_books?: { title?: string; available_copies?: number } })
                  .library_books;
                      ...
                      {r.status === "pending" && (
                        <>
                          <Button
                            ...
                            variant="outline"
                            disabled={!avail}
                            title={avail ? "Check out to reserver" : "No copies available"}
                            onClick={async () => {
                              try {
                                const due = new Date();
                                due.setDate(due.getDate() + 14);
                                await fulfil({
                                  data: { id: r.id, due_date: due.toISOString().slice(0, 10) },
                                });
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              await update({
                                data: { id: r.id, status: "cancelled" as ReservationStatus },
                              });
                              router.invalidate();
                            }}
                          >
