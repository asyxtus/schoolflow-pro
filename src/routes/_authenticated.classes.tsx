            ...
            <Card key={c.className} className="relative overflow-hidden">
              ...
              <CardContent className="space-y-4 p-5 pl-6">
                <div className="flex items-start justify-between">
                  ...
                  <Badge variant="secondary">{c.active}/{c.total} active</Badge>
                </div>

                <div className="grid grid-cols-3 gap-3 text-xs">
                  <Stat icon={<Users className="h-3.5 w-3.5" />} label="Learners" value={String(c.total)} />
                  <Stat icon={<TrendingUp className="h-3.5 w-3.5" />} label="Attend." value={`${c.avgAttendance}%`} />
                  <Stat icon={<Wallet className="h-3.5 w-3.5" />} label="Owed" value={fmtFcfa(c.outstanding)} />
                </div>

                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  <span>M {c.male} · F {c.female}</span>
                  <Button asChild size="sm" variant="ghost" className="h-7 px-2 text-xs">
                    <Link to="/students" search={{ q: "", class: c.className, fees: "all", status: "all" }}>
                      View roster →
                    </Link>
                  </Button>
                </div>
