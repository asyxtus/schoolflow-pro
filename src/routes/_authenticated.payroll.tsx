...
function PayrollPage() {
  ...
  async function submit() {
    try {
      const r = await create({ data: { period, notes } });
      toast.success("Payroll run created");
      setOpen(false);
      router.navigate({ to: "/payroll/$runId", params: { runId: r.id } });
    } catch (e) {
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h1 className="flex items-center gap-2 text-3xl font-semibold tracking-tight">
            <Banknote className="h-7 w-7 text-primary" /> Payroll
          </h1>
          <p className="text-sm text-muted-foreground">
            Monthly payroll runs generate payslips for every active staff member.
          </p>
        </div>
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-muted-foreground">
              Draft payslips will be generated for every active staff member using their base salary
              and active recurring allowances/deductions.
            </p>
          </div>
          <DialogFooter>
