...
function DayClosePage() {
  ...
  const closeMut = useMutation({
    mutationFn: async () =>
      closeFn({
        data: { date, counted_cash: countedCash ? Number(countedCash) : undefined, notes },
      }),
    onSuccess: () => {
      toast.success("Day closed");
      qc.invalidateQueries({ queryKey: ["day-close"] });
      setCountedCash("");
      setNotes("");
    },
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-6 py-8 space-y-6">
      <PageHeader
        ...
        description="End-of-day reconciliation — locks that day's payments once closed."
      />
      ...
      <div className="flex items-end gap-3">
        ...
        {closure && (
          <Badge variant="secondary" className="mb-2">
            <Lock className="mr-1 h-3 w-3" /> Closed on{" "}
        ...
        <Card>
          <CardContent className="p-6 space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              After closing, only the Principal can modify payments dated {date}. Enter counted cash
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <div
                  className={`mt-2 font-semibold ${variance === 0 ? "text-primary" : "text-amber-700"}`}
                >
                  {variance > 0 ? "+" : ""}
                  {fmt(variance)}
          ...
          <CardContent className="p-6 space-y-3">
            ...
            {closure.cash_variance !== null && closure.cash_variance !== 0 && (
              <div className="text-sm text-amber-700">
                Recorded variance: {fmt(closure.cash_variance)}
              </div>
            )}
            {closure.notes && (
              <div className="text-sm text-muted-foreground">Notes: {closure.notes}</div>
            )}
            <div>
              ...
              <Input
                ...
                placeholder="Why are you re-opening?"
              />
            </div>
            <Button
              variant="outline"
              onClick={() => reopenMut.mutate()}
              disabled={reopenMut.isPending || reopenReason.trim().length < 4}
            >
