...
function HRPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return staff.filter((r) =>
      `${r.first_name} ${r.last_name} ${r.matricule ?? ""} ${r.position} ${r.department ?? ""}`
        .toLowerCase()
        .includes(s),
    );
  }, [staff, q]);
  ...
  const totalPayroll = useMemo(
    () =>
      staff.filter((s) => s.status === "active").reduce((a, r) => a + (r.base_salary_fcfa ?? 0), 0),
    [staff],
  );
  ...
  function edit(r: StaffRow) {
    setForm({
      ...
      position: r.position as StaffPosition,
      department: r.department,
      contract_type: r.contract_type as ContractType,
      status: r.status as StaffStatus,
      hire_date: r.hire_date,
      end_date: r.end_date,
      base_salary_fcfa: r.base_salary_fcfa,
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="flex items-center gap-2 text-3xl font-semibold tracking-tight">
            <Briefcase className="h-7 w-7 text-primary" /> HR &amp; Staff
          </h1>
          <p className="text-sm text-muted-foreground">
            Manage personnel records, contracts, and salary base.
          </p>
        </div>
        ...
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <div className="relative w-72">
            ...
            <Input
              ...
              placeholder="Search by name, matricule, position…"
              className="pl-9"
            />
          </div>
        </CardHeader>
              ...
              {!filtered.length && (
                <tr>
                  <td colSpan={7} className="px-3 py-8 text-center text-muted-foreground">
                    No staff yet. Click <b>Add staff</b> to create the first record.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <DialogContent className="max-w-3xl">
          ...
          <div className="grid gap-3 md:grid-cols-2 max-h-[70vh] overflow-y-auto pr-2">
            ...
            <Field label="Position">
              <Select
                value={form.position}
                onValueChange={(v) => setForm({ ...form, position: v as StaffPosition })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <Field label="Base monthly salary (FCFA)">
              <Input
                type="number"
                value={form.base_salary_fcfa}
                onChange={(e) =>
                  setForm({ ...form, base_salary_fcfa: Number(e.target.value) || 0 })
                }
              />
            </Field>
          ...
          <DialogFooter>
            {form.id && (
              <Button asChild variant="secondary">
                <Link to="/hr/$staffId" params={{ staffId: form.id }}>
                  Open profile
                </Link>
              </Button>
            )}
