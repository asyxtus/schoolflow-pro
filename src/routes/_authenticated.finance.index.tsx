...
function FinancePage() {
  ...
  const schoolQ = useQuery({ queryKey: ["current-school"], queryFn: () => getCurrentSchool() });
  ...
  const refetchAll = () => {
    ...
    // A payment recorded here can change a student's balance shown on their profile
    // page or in this dialog's own per-student billing cache — both are keyed by
    // student id, so a plain key match above would miss them. Invalidate by prefix.
    qc.invalidateQueries({ queryKey: ["student"], exact: false });
    qc.invalidateQueries({ queryKey: ["student-billing"], exact: false });
  };
    ...
    <div className="mx-auto w-full max-w-7xl px-6 py-8 space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          value={fmt(summaryQ.data?.collected ?? 0)}
          icon={Wallet}
        />
        <StatCard
          label="This month"
          value={fmt(summaryQ.data?.thisMonth ?? 0)}
          icon={TrendingUp}
          tone="accent"
        />
        ...
        <StatCard
          ...
          value={String(summaryQ.data?.students ?? 0)}
          icon={Users}
        />
      </div>
      ...
      <Tabs defaultValue="payments">
        ...
        <TabsContent value="payments" className="space-y-3">
          <div className="flex flex-wrap items-end gap-2">
            ...
            <div className="grid gap-1">
              ...
              <Select
                value={filter.method}
                onValueChange={(v) => setFilter({ ...filter, method: v as PaymentMethod | "all" })}
              >
            ...
            <div className="ml-auto flex items-end gap-2">
              ...
              <RecordPaymentDialog
                ...
                onSubmit={async (v) => {
                  ...
                  toast.success(
                    res?.credit
                      ? `Payment recorded · ${fmt(res.credit)} kept as credit`
                      : "Payment recorded",
                    {
                      action: res?.id
                        ? { label: "Print receipt", onClick: () => openReceipt(res.id!) }
                        : undefined,
                    },
                  );
                }}
              ...
              <CardContent className="p-0">
                    ...
                    return (
                      <div key={p.id} className="flex items-center gap-3 p-4">
                        <div className="flex-1">
                          ...
                          {p.note && (
                            <div className="mt-1 text-sm text-foreground/80">{p.note}</div>
                          )}
                          {(p as { voided?: boolean; void_reason?: string | null }).voided &&
                            (p as { void_reason?: string | null }).void_reason && (
                              <div className="mt-1 text-xs text-destructive">
                                Void reason: {(p as { void_reason?: string | null }).void_reason}
                              </div>
                            )}
                        </div>
                        <div className="text-right">
                          <div
                            className={`font-semibold ${(p as { voided?: boolean }).voided ? "text-muted-foreground line-through" : "text-primary"}`}
                          >
                            {fmt(p.amount_fcfa)}
                          </div>
                        </div>
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              const reason = window.prompt(
                                "Reason for voiding this payment (audit-logged)?",
                              );
                ...
                <div className="border-t p-3 text-right text-sm">
                  ...
                  <span className="font-semibold text-foreground">
                    {fmt(paysQ.data.reduce((a, r) => a + r.amount_fcfa, 0))}
                  </span>
                </div>
              </CardContent>
            </Card>
        ...
        <TabsContent value="invoices" className="space-y-3">
          <div className="flex flex-wrap justify-end gap-2">
            <BulkAssignDialog
              ...
              onSubmit={async (v) => {
                ...
                toast.success(
                  r.skipped > 0
                    ? `Assigned to ${r.count} students · ${r.skipped} already billed, skipped`
                    : `Assigned to ${r.count} students`,
                );
              }}
            />
            <InvoiceDialog
          ...
          {!invQ.data?.length ? (
            <Card>
              <CardContent className="p-8 text-center text-sm text-muted-foreground">
                No fee assignments yet. Create one or bulk-assign a fee structure to a class.
              </CardContent>
            </Card>
          ) : (
            <Card>
              <CardContent className="p-0">
                <div className="divide-y">
                  {invQ.data.map((r) => {
                    ...
                    const status = (r.status ?? "unpaid") as
                      ...
                      <div key={r.id} className="flex items-center gap-3 p-4">
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-xs text-muted-foreground">
                            ...
                            {Number(r.discount_fcfa ?? 0) > 0 &&
                              ` · -${fmt(Number(r.discount_fcfa))} discount`}
                          </div>
                        </div>
                        {paid > 0 ? (
                          <VoidInvoiceButton
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              await delInv({ data: { id: r.id as string } });
                              refetchAll();
                            }}
                          >
                            <Trash2 className="h-4 w-4 text-muted-foreground" />
        ...
        <TabsContent value="structures" className="space-y-3">
          <div className="flex justify-end">
            <FeeStructureDialog
              defaultAcademicYear={schoolQ.data?.school?.current_academic_year ?? undefined}
              onSubmit={async (v) => {
                const r = await upsertFee({ data: v });
                qc.invalidateQueries({ queryKey: ["fee-structures"] });
                qc.invalidateQueries({ queryKey: ["approval-requests"] });
                toast.success(
                  r.pendingApproval
                    ? "Submitted for diocese approval — tuition won't change until it's reviewed"
                    : "Fee structure saved",
                );
              }}
            />
          ...
          {!feesQ.data?.length ? (
            ...
            <Card>
              <CardContent className="p-0">
                <div className="divide-y">
                  {feesQ.data.map((f) => {
                    const inst = Array.isArray(f.installments)
                      ? (f.installments as unknown as FeeInstallment[])
                      : [];
                    const kind = ((f.kind as FeeKind | null | undefined) ?? "tuition") as FeeKind;
                    const req = f.required_at_registration;
                    const currentYear = schoolQ.data?.school?.current_academic_year;
                    const isStaleYear =
                      !!currentYear && !!f.academic_year && f.academic_year !== currentYear;
                    return (
                      <div key={f.id} className="p-4">
                        <div className="flex items-center gap-3">
                          ...
                          {currentYear && isStaleYear && (
                            <Badge variant="outline" className="text-muted-foreground">
                              Not this year — won't bill new admissions
                            </Badge>
                          )}
                          <div className="flex-1">
                            <div className="font-medium">{f.label}</div>
                            {f.academic_year && (
                              <div className="text-xs text-muted-foreground">{f.academic_year}</div>
                            )}
                          </div>
...
function VoidInvoiceButton({ id, onVoided }: { id: string; onVoided: () => void }) {
  const voidFn = useServerFn(voidStudentFee);
  ...
  const submit = async () => {
    if (reason.trim().length < 4) {
      toast.error("Enter a reason (at least 4 characters)");
      return;
    }
    setBusy(true);
    try {
      await voidFn({ data: { id, reason: reason.trim() } });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="icon" variant="ghost" aria-label="Void invoice" title="Void invoice">
          <Trash2 className="h-4 w-4 text-muted-foreground" />
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-md">
        ...
        <p className="text-sm text-muted-foreground">
          This invoice has a payment applied to it, so it can't be deleted outright — only voided,
          an auditable action. Any money already applied to it will become unallocated credit on the
          student's account, ready to apply elsewhere.
        </p>
        <div className="grid gap-1.5">
          ...
          <Textarea
            ...
            placeholder="e.g. Billed to the wrong class by mistake"
          />
        </div>
        <DialogFooter>
...
function RecordPaymentDialog({
  ...
  const submit = async () => {
    if (!studentId || amt <= 0) {
      toast.error("Pick a student and an amount");
      return;
    }
    if (overAllocated) {
              ...
              <SelectContent>
                {students.map((s) => (
                  <SelectItem key={s.id} value={s.id}>
                    ...
                    {s.fee_balance > 0 ? `(owes ${fmt(s.fee_balance)})` : ""}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
              ...
              <div className="mb-2 flex items-center justify-between text-xs">
                <span className="text-muted-foreground">
                  {billingQ.isLoading
                    ...
                    : open_.length === 0
                      ? "No open invoices"
                      : "Applied to (oldest due first — you can adjust)"}
                </span>
                ...
                <>
                  ...
                  <div className="mt-2 space-y-1 border-t border-border pt-2 text-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={
                          overAllocated ? "font-semibold text-destructive" : "font-semibold"
                        }
                      >
                        {fmt(allocatedTotal)}
                      </span>
...
function FeeStructureDialog({
  ...
  const [year, setYear] = useState(initial?.academic_year ?? defaultAcademicYear ?? "");
  ...
  const addRow = () =>
    setInstallments((p) => [
      ...p,
      { label: `Installment ${p.length + 1}`, amount_fcfa: 0, due_date: "" },
    ]);
  const removeRow = (idx: number) => setInstallments((p) => p.filter((_, i) => i !== idx));

  const submit = async () => {
    if (!className || !label) {
      toast.error("Class and label are required");
      return;
    }
    if (!total) {
      toast.error("Add at least one installment amount");
      return;
    }
    setBusy(true);
    try {
      const cleaned = installments.map((i) => ({
        label: i.label,
        amount_fcfa: Number(i.amount_fcfa || 0),
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{initial?.id ? "Edit fee structure" : "New fee structure"}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          ...
          <div className="grid gap-1.5">
            ...
            <Input
              ...
              placeholder={kind === "registration" ? "Registration fee" : "Tuition — 2025/2026"}
            />
          </div>
                ...
                <div key={idx} className="grid grid-cols-[1fr_140px_160px_auto] gap-2 items-end">
                  ...
                  <div className="grid gap-1">
                    ...
                    <Input
                      type="number"
                      min="0"
                      value={row.amount_fcfa || ""}
                      onChange={(e) => update(idx, { amount_fcfa: Number(e.target.value) })}
                    />
                  </div>
                  <div className="grid gap-1">
  ...
  const rows = useMemo(() => {
    ...
    return students
      .filter(
        (r) =>
          !s ||
          `${r.first_name} ${r.last_name} ${r.matricule ?? ""} ${r.class_name ?? ""}`
            .toLowerCase()
            .includes(s),
      )
    ...
    <div className="space-y-3">
      <div className="flex items-center gap-3">
        ...
        <div className="text-sm text-muted-foreground ml-auto">
          Total outstanding: <span className="font-semibold text-foreground">{fmt(total)}</span>
        </div>
      </div>
      <Card>
        <CardContent className="p-0">
          <div className="divide-y">
            {rows.map((s) => (
              <div key={s.id} className="flex items-center gap-3 p-3">
                ...
                <div
                  className={
                    s.fee_balance > 0 ? "font-semibold text-destructive" : "text-muted-foreground"
                  }
                >
                  {fmt(s.fee_balance ?? 0)}
                </div>
            ...
            {!rows.length && (
              <div className="p-8 text-center text-sm text-muted-foreground">No students.</div>
            )}
          </div>
        </CardContent>
      </Card>
...
function InvoiceDialog({
  ...
  const applyStructure = (id: string) => {
    ...
    if (s) {
      setLabel(s.label);
      setAmount(String(s.amount_fcfa));
    }
  };

  const submit = async () => {
    const amt = Number(amount);
    if (!studentId || !label || !amt) {
      toast.error("Fill student, label and amount");
      return;
    }
    if (Number(discount) > 0 && note.trim().length < 4) {
      toast.error("A note explaining the discount is required (at least 4 characters)");
      return;
    }
    setBusy(true);
    try {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
                ...
                <SelectContent>
                  {feeStructures.map((f) => (
                    <SelectItem key={f.id} value={f.id}>
                      {f.class_name} · {f.label} — {fmt(f.amount_fcfa)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <div className="grid gap-1.5">
            <Label>Note {Number(discount) > 0 ? "(required for discount)" : "(optional)"}</Label>
            <Textarea rows={2} value={note} onChange={(e) => setNote(e.target.value)} />
          </div>
        </div>
        <DialogFooter>
...
function BulkAssignDialog({
  ...
  const submit = async () => {
    ...
    if (!structureId && (!label || !amount)) {
      toast.error("Pick a fee structure or provide label + amount");
      return;
    }
    setBusy(true);
    try {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Assign fee to a whole class</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          <div className="grid gap-1.5">
            <Label>Class</Label>
