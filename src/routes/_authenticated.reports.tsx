...
import { listSubjects, listSubjectsForClassName } from "@/lib/subjects.functions";
...
function ReportsPage() {
  ...
  const fetchClassSubjects = useServerFn(listSubjectsForClassName);
  const fetchAllSubjects = useServerFn(listSubjects);

  const classesQ = useQuery({ queryKey: ["class-names"], queryFn: () => listClasses() });
  const classSubjectsQ = useQuery({
    queryKey: ["subjects-for-class", className],
    queryFn: () => fetchClassSubjects({ data: { className } }),
    enabled: !!className,
  });
  const allSubjectsQ = useQuery({
    queryKey: ["subjects"],
    queryFn: () => fetchAllSubjects({ data: {} }),
  });
  // Prefer subjects actually assigned to this class; fall back to the
  // school-wide list for a class that hasn't had subjects configured yet.
  const subjectOptions =
    (classSubjectsQ.data?.length ? classSubjectsQ.data : allSubjectsQ.data) ?? [];
  ...
  const saveMut = useMutation({
    mutationFn: (v: { studentId: string; ca?: number | null; exam?: number | null }) =>
      saveGrade({
        data: { studentId: v.studentId, sequence, subject, ca_score: v.ca, exam_score: v.exam },
      }),
  ...
  const rows = useMemo(() => {
    return students.map((s) => {
      ...
      const total =
        ca != null && exam != null ? Math.round((ca * 0.4 + exam * 0.6) * 10) / 10 : null;
        ...
        <CardContent className="flex flex-wrap items-end gap-3 p-4">
          ...
          <div className="grid gap-1.5">
            ...
            {subjectOptions.length === 0 ? (
              <div className="flex h-9 w-52 items-center rounded-md border border-dashed px-3 text-xs text-muted-foreground">
                {className ? (
                  <>
                    No subjects for this class —{" "}
                    <Link to="/classes/manage" className="ml-1 underline">
                      set up in Classes
                    </Link>
                  </>
                ) : (
      ...
      {!className ? (
        <Card>
          <CardContent className="p-10 text-center text-sm text-muted-foreground">
            Select a class to enter scores.
          </CardContent>
        </Card>
      ) : !students.length ? (
        <Card>
...
function ScoreRow({
  ...
  const commit = () => {
    ...
    if (
      (nextCa != null && (isNaN(nextCa) || nextCa < 0 || nextCa > 100)) ||
      (nextExam != null && (isNaN(nextExam) || nextExam < 0 || nextExam > 100))
    )
      return;
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        {total != null ? (
          <Badge variant={total >= 50 ? "default" : "secondary"}>{total}</Badge>
        ) : (
