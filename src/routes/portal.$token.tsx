...
export const Route = createFileRoute("/portal/$token")({
  head: () => ({
    meta: [{ title: "Parent Portal" }, { name: "robots", content: "noindex" }],
  }),
  loader: ({ context, params }) => context.queryClient.ensureQueryData(bundleQuery(params.token)),
...
function InvalidLink() {
  return (
    <div className="mx-auto max-w-lg p-8 text-center">
      ...
      <p className="mt-2 text-sm text-muted-foreground">Ask the school office for a new link.</p>
    </div>
  );
}
    ...
    <div className="mx-auto w-full max-w-4xl p-4 md:p-8">
      <header className="mb-6">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
          ...
          <Badge variant={balance > 0 ? "destructive" : "secondary"}>Balance {fmt(balance)}</Badge>
        </div>
      </header>
