...
import { getClassSubjectAssignments } from "@/lib/subjects.functions";
...
function TimetablePage() {
  ...
  const fetchAssignments = useServerFn(getClassSubjectAssignments);

  const classesQ = useQuery({ queryKey: ["class-names"], queryFn: () => listClasses() });
  const assignmentsQ = useQuery({
    queryKey: ["class-subject-assignments", className],
    queryFn: () => fetchAssignments({ data: { className } }),
    enabled: !!className,
  });
  const ttQ = useQuery({
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["timetable", className] });
      toast.success("Saved");
      setEditing(null);
    },
                        ...
                        <td key={di} className="p-2 align-top">
                          {slot ? (
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-16 w-full border border-dashed text-muted-foreground"
                              onClick={() => setEditing({ day, period: p })}
                            >
...
function SlotDialog({
  ...
  day: number;
  period: number;
  assignments: { subject: string; teacherName: string }[];
  onSave: (v: { subject: string; teacher: string; room: string }) => void;
  saving: boolean;
}) {
  const [subject, setSubject] = useState(initial?.subject ?? "");
  const [teacher, setTeacher] = useState(initial?.teacher ?? "");
  const [teacherTouched, setTeacherTouched] = useState(!!initial?.teacher);
  const [room, setRoom] = useState(initial?.room ?? "");

  const pickSubject = (v: string) => {
    setSubject(v);
    if (!teacherTouched) {
      const assigned = assignments.find((a) => a.subject === v)?.teacherName ?? "";
      setTeacher(assigned);
    }
  };
        ...
        <div className="grid gap-3">
          <div className="grid gap-1.5">
            ...
            {assignments.length === 0 ? (
              <div className="rounded-md border border-dashed px-3 py-2 text-xs text-muted-foreground">
                This class has no subjects set up yet — add them in Classes → Manage first.
              </div>
            ) : (
              <Select value={subject} onValueChange={pickSubject}>
                <SelectTrigger>
                  <SelectValue placeholder="Select subject" />
                </SelectTrigger>
                <SelectContent>
                  {assignments.map((a) => (
                    <SelectItem key={a.subject} value={a.subject}>
                      {a.subject}
                    </SelectItem>
                  ))}
                </SelectContent>
          ...
          <div className="grid gap-1.5">
            ...
            <Input
              ...
              placeholder={
                assignments.find((a) => a.subject === subject)?.teacherName
                  ? undefined
                  : "No teacher assigned to this subject yet"
              }
            />
            <p className="text-xs text-muted-foreground">
              Auto-filled from the subject's assigned teacher. Change it here only for a substitute
              — it won't affect the official assignment in Classes → Manage.
            </p>
          </div>
          <div className="grid gap-1.5">
            <Label>Room</Label>
