...
import {
  ...
  listStudentWalletBalances,
  getSchoolWalletDefaults,
  updateSchoolWalletDefaults,
  updateStudentWalletLimits,
  getStudentWalletContext,
  type WalletMethod,
  type WalletKind,
  type WalletLimits,
...
function WalletPage() {
  ...
  const [balFilter, setBalFilter] = useState<{ q: string; className: string }>({
    q: "",
    className: "",
  });
  ...
  const balQ = useQuery({
    ...
    queryFn: () =>
      balancesFn({ data: { q: balFilter.q, className: balFilter.className || undefined } }),
  });
  ...
  const saveLimits = useMutation({
    ...
    onSuccess: () => {
      toast.success("Student limits saved");
      refetchAll();
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const classNames = useMemo(() => {
    ...
    (balQ.data ?? []).forEach((r) => {
      if (r.class_name) s.add(r.class_name);
    });
    ...
    <div className="space-y-6 p-6">
        ...
        actions={
          <div className="flex gap-2">
            <DefaultsDialog
              ...
              onSave={(v) => saveDefaults.mutate(v)}
            />
            <TransactionDialog
              students={balQ.data ?? []}
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <StatCard
          ...
          value={fmt(summary?.depositsThisMonth ?? 0)}
          icon={ArrowDownCircle}
        />
        <StatCard
          label="Withdrawals this month"
          value={fmt(summary?.withdrawalsThisMonth ?? 0)}
          icon={ArrowUpCircle}
        />
      </div>
        ...
        <TabsContent value="transactions" className="space-y-4">
          <Card>
            <CardContent className="flex flex-wrap gap-3 p-4">
              <Input
                placeholder="Search student / matricule / reference"
                value={filter.q}
              ...
              <Select
                value={filter.kind}
                onValueChange={(v) => setFilter((f) => ({ ...f, kind: v as WalletKind | "all" }))}
              >
                    ...
                    {(txQ.data ?? []).map((t) => {
                      ...
                      return (
                        <tr key={t.id} className="border-t border-border/60">
                          ...
                          <td className="px-4 py-3">
                            <Badge
                              variant={t.kind === "deposit" ? "default" : "secondary"}
                          ...
                          <td
                            className={`px-4 py-3 text-right font-mono ${t.kind === "deposit" ? "text-emerald-600" : "text-amber-700"}`}
                          >
                            {t.kind === "deposit" ? "+" : "−"}
                            {fmt(t.amount_fcfa)}
                          </td>
                            ...
                            {tx.over_limit ? (
                              <div className="flex items-center gap-1 text-xs">
                                ...
                                <span
                                  ...
                                  title={tx.guardian_approval_note ?? ""}
                                >
                                  Guardian OK
                                </span>
                              </div>
          ...
          <Card>
            <CardContent className="flex flex-wrap gap-3 p-4">
              <Input
                ...
                value={balFilter.q}
                onChange={(e) => setBalFilter((f) => ({ ...f, q: e.target.value }))}
                className="w-72"
              />
              <Select
                value={balFilter.className || "all"}
                onValueChange={(v) =>
                  setBalFilter((f) => ({ ...f, className: v === "all" ? "" : v }))
                }
              >
                      ...
                      return (
                        <tr key={s.id} className="border-t border-border/60">
                          ...
                          <td className="px-4 py-3 text-right font-mono">
                            {fmt(s.wallet_balance ?? 0)}
                          </td>
                          <td className="px-4 py-3">
                            <LimitsDialog
                              student={{ id: s.id, name: `${s.first_name} ${s.last_name}` }}
                              limits={{
                                per_txn: st.wallet_per_txn_limit,
                                daily: st.wallet_daily_limit,
                                weekly: st.wallet_weekly_limit,
                                monthly: st.wallet_monthly_limit,
                              }}
                              defaults={defaultsQ.data ?? null}
...
function TransactionDialog({
  ...
  const reasons = useMemo(() => {
    ...
    const e = currentCtx.effective,
      u = currentCtx.usage;
    const r: string[] = [];
    if (e.per_txn != null && amt > e.per_txn)
      r.push(`Amount exceeds per-transaction limit (${fmt(e.per_txn)})`);
    if (e.daily != null && u.day.count + 1 > e.daily)
      r.push(`Exceeds daily frequency (${u.day.count}/${e.daily} today)`);
    if (e.weekly != null && u.week.count + 1 > e.weekly)
      r.push(`Exceeds weekly frequency (${u.week.count}/${e.weekly} this week)`);
    if (e.monthly != null && u.month.count + 1 > e.monthly)
      r.push(`Exceeds monthly frequency (${u.month.count}/${e.monthly} this month)`);
    return r;
  }, [kind, currentCtx, amt]);

  const reset = () => {
    setStudentId(presetStudent?.id ?? "");
    setKind(presetKind ?? "deposit");
    setAmount("");
    setMethod("cash");
    setReference("");
    setNote("");
    ...
    >
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {presetStudent
              ? `${kind === "deposit" ? "Deposit to" : "Withdraw from"} ${presetStudent.first_name} ${presetStudent.last_name}`
        ...
        <div className="grid gap-3">
          ...
          {kind === "withdrawal" && currentCtx && (
            <div className="rounded-md border border-border/60 bg-muted/30 p-3 text-xs">
              <div className="mb-1 font-medium text-foreground">
                Wallet balance: {fmt(currentCtx.student.wallet_balance ?? 0)}
              </div>
              <div className="grid grid-cols-2 gap-x-3 gap-y-0.5 text-muted-foreground">
                <span>Per-txn limit</span>
                <span>
                  {currentCtx.effective.per_txn != null
                    ? fmt(currentCtx.effective.per_txn)
                    : "None"}
                </span>
                <span>Today</span>
                <span>
                  {currentCtx.usage.day.count} / {currentCtx.effective.daily ?? "∞"}
                </span>
                <span>This week</span>
                <span>
                  {currentCtx.usage.week.count} / {currentCtx.effective.weekly ?? "∞"}
                </span>
                <span>This month</span>
                <span>
                  {currentCtx.usage.month.count} / {currentCtx.effective.monthly ?? "∞"}
                </span>
              </div>
            </div>
          )}
            ...
            <div className="rounded-md border border-amber-500/40 bg-amber-500/10 p-3 text-xs">
              ...
              <div className="mt-2 grid gap-1">
                ...
                <Input
                  ...
                  placeholder="e.g. Phone call with mother at 10:30"
                />
              </div>
            </div>
          )}
          <div className="grid gap-1">
            ...
            <Input
              ...
              placeholder="MoMo TxID, cheque #, etc."
            />
          </div>
          <div className="grid gap-1">
        ...
        <DialogFooter>
          <Button
            onClick={submit}
            disabled={reasons.length > 0 && (!guardianOk || !guardianNote.trim())}
          >
            Save transaction
          </Button>
        </DialogFooter>
...
function QuickTxButtons({
  ...
  student: StudentLite;
  onSubmit: (v: TxInput) => void;
  fetchContext: (id: string) => Promise<WalletCtx>;
}) {
  return (
...
function DefaultsDialog({
  ...
  defaults: WalletLimits | null;
  onSave: (v: WalletLimits) => void;
}) {
  const [open, setOpen] = useState(false);
  const [v, setV] = useState<WalletLimits>({
    per_txn: null,
    daily: null,
    weekly: null,
    monthly: null,
  ...
  return (
    <Dialog
      ...
      onOpenChange={(o) => {
        setOpen(o);
        if (o) setV(defaults ?? { per_txn: null, daily: null, weekly: null, monthly: null });
      }}
    >
      <DialogTrigger asChild>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>School-wide default withdrawal limits</DialogTitle>
        </DialogHeader>
        <p className="text-xs text-muted-foreground">
          Leave blank to allow unrestricted withdrawals. Per-student overrides always win.
        </p>
        <div className="grid grid-cols-2 gap-3">
          <LimitInput
            label="Max per transaction (FCFA)"
            value={v.per_txn}
            onChange={(x) => setV((s) => ({ ...s, per_txn: x }))}
          />
          <LimitInput
            label="Max withdrawals per day"
            value={v.daily}
            onChange={(x) => setV((s) => ({ ...s, daily: x }))}
          />
          <LimitInput
            label="Max withdrawals per week"
            value={v.weekly}
            onChange={(x) => setV((s) => ({ ...s, weekly: x }))}
          />
          <LimitInput
            label="Max withdrawals per month"
            value={v.monthly}
            onChange={(x) => setV((s) => ({ ...s, monthly: x }))}
          />
        </div>
        <DialogFooter>
          <Button
...
function LimitsDialog({
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Withdrawal limits — {student.name}</DialogTitle>
        </DialogHeader>
        <p className="text-xs text-muted-foreground">
          Leave a field blank to inherit the school default
          {defaults
            ? ` (per-txn ${defaults.per_txn ?? "∞"}, ${defaults.daily ?? "∞"}/day, ${defaults.weekly ?? "∞"}/wk, ${defaults.monthly ?? "∞"}/mo)`
            : ""}
          .
        </p>
        <div className="grid grid-cols-2 gap-3">
        ...
        <DialogFooter>
          <Button
            variant="outline"
            onClick={() => {
              onSave({ per_txn: null, daily: null, weekly: null, monthly: null });
              setOpen(false);
            }}
          >
            Clear overrides
