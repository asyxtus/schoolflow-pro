...
import { Wallet, ArrowDownCircle, ArrowUpCircle, Users, Plus, Trash2 } from "lucide-react";
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    ...
                    {!txQ.data?.length && (
                      <tr><td colSpan={8} className="px-4 py-10 text-center text-muted-foreground">No transactions yet.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    ...
                    {!balQ.data?.length && (
                      <tr><td colSpan={5} className="px-4 py-10 text-center text-muted-foreground">No students found.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
...
type TxInput = { student_id: string; kind: WalletKind; amount_fcfa: number; method: WalletMethod; reference?: string; note?: string; occurred_at?: string };

function TransactionDialog({ students, onSubmit, presetStudent, presetKind, triggerLabel, triggerVariant }: {
  ...
  const reset = () => { setStudentId(presetStudent?.id ?? ""); setKind(presetKind ?? "deposit"); setAmount(""); setMethod("cash"); setReference(""); setNote(""); };

  const submit = () => {
    const amt = Number(amount);
    if (!studentId) return;
    if (!amt || amt <= 0) return;
    onSubmit({ student_id: studentId, kind, amount_fcfa: amt, method, reference: reference || undefined, note: note || undefined });
    setOpen(false);
    reset();
  };
...
function QuickTxButtons({ student, onSubmit }: { student: StudentLite; onSubmit: (v: TxInput) => void }) {
  return (
    <div className="flex justify-end gap-2">
      <TransactionDialog students={[]} presetStudent={student} presetKind="deposit" onSubmit={onSubmit} triggerLabel="Deposit" triggerVariant="outline" />
      <TransactionDialog students={[]} presetStudent={student} presetKind="withdrawal" onSubmit={onSubmit} triggerLabel="Withdraw" triggerVariant="ghost" />
    </div>
  );
}
