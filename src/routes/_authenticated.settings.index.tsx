...
function SettingsPage() {
  ...
  useEffect(() => {
    if (data?.school) {
      setSchool({
        ...
        current_academic_year: data.school.current_academic_year ?? "",
      });
    }
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-6 py-8 space-y-6">
      <PageHeader title="Settings" description="School profile and your account" />
        ...
        <Card>
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Current academic year">
              <Input
                value={school.current_academic_year}
                onChange={(e) => setSchool({ ...school, current_academic_year: e.target.value })}
                placeholder="e.g. 2026/2027"
              />
              <p className="mt-1 text-xs text-muted-foreground">
                Used to pick which year's fee structures apply when a new student is admitted. Keep
                this in the same format you use on your fee structures (e.g. "2026/2027").
              </p>
            </Field>
