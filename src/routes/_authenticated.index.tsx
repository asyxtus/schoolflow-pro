...
export const Route = createFileRoute("/_authenticated/")({
  ...
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Live pulse of enrolment, attendance, collections and alerts for your school.",
      },
    ],
  }),
});
...
function DashboardPage() {
  ...
  const subtitle =
    [school?.name, locale].filter(Boolean).join(" · ") || "Set up your school in Settings";
      ...
      <Card className="mb-6">
        ...
        <CardContent className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <p
              className={`text-[11px] ${
                t.exeatsOverdue > 0 ? "text-destructive font-medium" : "text-muted-foreground"
              }`}
            >
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            <div>
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">Live · billed vs collected</p>
            </div>
          ...
          <CardContent className="divide-y divide-border">
            {stats.upcoming.length === 0 ? (
              ...
              stats.upcoming.slice(0, 6).map((u) => (
                <div key={u.id} className="flex items-center justify-between py-2.5 text-sm gap-3">
                  <div className="min-w-0">
                  ...
                  <div className="text-right shrink-0">
                    <p className="tabular-nums text-foreground">{formatFCFA(u.amount_fcfa)}</p>
                    <p className="text-xs text-muted-foreground">
