...
function NewStudentPage() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async ({ id, matricule, invoices }) => {
      await qc.invalidateQueries({ queryKey: ["students"] });
      await qc.invalidateQueries({ queryKey: ["dashboard-stats"] });
      await qc.invalidateQueries({ queryKey: ["student-fees"] });
      toast.success(matricule ? `Student added — matricule ${matricule}` : "Student added");
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-6 py-8">
      ...
      <form onSubmit={submit} className="space-y-6">
          ...
          <CardContent className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Fee balance (FCFA)">
              <Input
                ...
                onChange={(e) => set("feeBalance", e.target.value)}
              />
            </Field>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Primary guardian (optional)</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent>
            <Textarea
              ...
              placeholder="Health, boarding, scholarships…"
            />
          </CardContent>
        </Card>
...
function InitialPaymentDialog({
  ...
  const submit = async () => {
    ...
    const note =
      picked.length > 0 ? `Applied to: ${picked.map((p) => p.label).join("; ")}` : undefined;
    setBusy(true);
    try {
      await recordPayment({
        data: {
        ...
        <div className="space-y-3">
          <div className="rounded-md border border-border bg-muted/30 p-3">
            ...
            <div className="space-y-1.5 text-sm">
              {invoices.map((i, idx) => (
                <label
                  key={idx}
                  className="flex items-center justify-between gap-3 cursor-pointer rounded px-1 py-0.5 hover:bg-muted/60"
                >
          ...
          <div className="grid gap-1.5">
            ...
            <Input
              ...
              placeholder="MoMo txn ID, receipt no."
            />
          </div>
        </div>
        <DialogFooter>
