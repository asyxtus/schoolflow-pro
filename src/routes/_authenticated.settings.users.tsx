...
import {
  listStaff, listInvitations, createInvitation, revokeInvitation,
  updateStaffRole, removeStaff, MANAGEABLE_ROLES, type AppRole,
...
function UsersPage() {
  ...
  const grouped = new Map<string, {
    user_id: string; full_name: string | null; email: string | null; roles: string[];
  }>();
  for (const r of staffQ.data?.staff ?? []) {
    const g = grouped.get(r.user_id) ?? { user_id: r.user_id, full_name: r.full_name, email: r.email, roles: [] };
    g.roles.push(r.role);
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-6 py-8 space-y-6">
      ...
      <form onSubmit={submit}>
        <Card>
          <CardHeader><CardTitle className="text-base flex items-center gap-2"><UserPlus className="h-4 w-4" />Invite staff</CardTitle></CardHeader>
          <CardContent className="grid gap-3 sm:grid-cols-[1fr_200px_auto]">
            <div className="space-y-1.5">
              <Label className="text-xs">Email</Label>
              <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@school.cm" />
            </div>
            <div className="space-y-1.5">
      ...
      <Card>
        ...
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {(invQ.data ?? []).length === 0 ? (
                <TableRow><TableCell colSpan={5} className="text-center text-muted-foreground py-6">No invitations yet</TableCell></TableRow>
              ) : (invQ.data ?? []).map((i) => (
                <TableRow key={i.id}>
                  <TableCell className="font-medium">{i.email}</TableCell>
                  <TableCell>{ROLE_LABEL[i.role] ?? i.role}</TableCell>
                  <TableCell>
                    <Badge variant={i.status === "pending" ? "secondary" : i.status === "accepted" ? "default" : "outline"}>
                      {i.status}
                    </Badge>
                  </TableCell>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">Staff ({users.length})</CardTitle></CardHeader>
        <CardContent className="p-0">
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {users.length === 0 ? (
                <TableRow><TableCell colSpan={4} className="text-center text-muted-foreground py-6">No staff yet</TableCell></TableRow>
              ) : users.map((u) => (
                <TableRow key={u.user_id}>
                        ...
                        <Badge key={r} variant="secondary" className="text-xs">
                          ...
                          {MANAGEABLE_ROLES.includes(r as AppRole) && (
                            <button
                              ...
                              onClick={() => roleMut.mutate({ user_id: u.user_id, role: r as AppRole, add: false })}
                              aria-label={`Remove ${r}`}
                            >×</button>
                          )}
                        </Badge>
                  ...
                  <TableCell className="text-right space-x-2">
                    <Select onValueChange={(v) => roleMut.mutate({ user_id: u.user_id, role: v as AppRole, add: true })}>
                      <SelectTrigger className="h-8 w-[150px] inline-flex"><SelectValue placeholder="Add role" /></SelectTrigger>
                      <SelectContent>
                    ...
                    {!u.roles.includes("principal") && (
                      <Button size="sm" variant="ghost" onClick={() => removeMut.mutate(u.user_id)}>
