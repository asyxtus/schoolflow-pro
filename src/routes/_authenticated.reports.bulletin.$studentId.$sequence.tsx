...
export const Route = createFileRoute("/_authenticated/reports/bulletin/$studentId/$sequence")({
    ...
    <div className="mx-auto w-full max-w-5xl px-6 py-6 print:px-0 print:py-0">
      ...
      <div className="rounded-lg border bg-white p-8 text-black shadow-sm print:border-0 print:shadow-none">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 text-sm md:grid-cols-4">
          <Info
            label="Name"
            value={`${student?.last_name ?? ""} ${student?.first_name ?? ""}`.trim()}
          />
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-4">
          <Summary
            ...
            value={`${nf(totals?.overallAvg ? totals.overallAvg / 5 : null)} / 20`}
          />
          <Summary
            label="Class average"
            value={`${nf(totals?.classAvgOverall ? totals.classAvgOverall / 5 : null)} / 20`}
          />
          <Summary
            label="Rank"
            value={totals?.overallRank ? `${totals.overallRank} / ${totals.classSize}` : "—"}
          />
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <label className="flex flex-col gap-1">
                ...
                <Input
                  type="number"
                  value={absJ}
                  onChange={(e) => setAbsJ(Number(e.target.value))}
                  className="h-8 print:border-0 print:p-0 print:shadow-none"
                />
              </label>
              <label className="flex flex-col gap-1">
                ...
                <Input
                  type="number"
                  value={absU}
                  onChange={(e) => setAbsU(Number(e.target.value))}
                  className="h-8 print:border-0 print:p-0 print:shadow-none"
                />
              </label>
            </div>
          ...
          <div className="rounded border p-3 text-sm">
            ...
            <div>
              Strongest subject: <b>{strongest?.subject ?? "—"}</b>{" "}
              {strongest?.mark != null ? `(${nf(strongest.mark / 5)}/20)` : ""}
            </div>
            <div>
              Weakest subject: <b>{weakest?.subject ?? "—"}</b>{" "}
              {weakest?.mark != null ? `(${nf(weakest.mark / 5)}/20)` : ""}
            </div>
          </div>
        </div>
