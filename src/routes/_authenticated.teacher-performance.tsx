  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-6 py-8 space-y-5">
      <PageHeader
        ...
        description="Attendance-taking, grade timeliness, class average and own punctuality."
      />
      ...
      {q.isLoading ? (
        ...
        <Card>
          <CardContent className="p-8 text-center text-sm text-muted-foreground">
            No teachers assigned to any subject yet. Assign teachers in Classes → Manage.
          </CardContent>
        </Card>
      ) : (
        <Card>
                    ...
                    <tr key={t.staff_id} className="border-t">
                      ...
                      <td className="p-3">
                        {p === null ? (
                          ...
                          <Badge
                            variant={p >= 80 ? "default" : p >= 50 ? "secondary" : "destructive"}
                          >
                            {p}%
                          </Badge>
                        )}
                        <div className="text-xs text-muted-foreground">
                          {t.attendance_taken} / {t.attendance_expected}
                        </div>
                      </td>
                      <td className="p-3">{t.grades_entered}</td>
                      <td className="p-3">
                        {t.class_average === null ? "—" : t.class_average.toFixed(2)}
                      </td>
                      <td className="p-3">
                        {t.punctuality_pct === null ? (
                          ...
                          <Badge
                            variant={
                              t.punctuality_pct >= 90
                                ? "default"
                                : t.punctuality_pct >= 70
                                  ? "secondary"
                                  : "destructive"
                            }
                          >
