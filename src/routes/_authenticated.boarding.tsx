...
import {
  ...
  upsertDormitory,
  deleteDormitory,
  listBoardingAssignments,
  assignBoardingStudent,
  releaseBoardingAssignment,
  getRollCall,
  saveRollCall,
  listExeats,
  createExeat,
...
function BoardingPage() {
  return (
    <div>
      <PageHeader
        title="Boarding (Internat)"
        description="Dormitories, roll call, exeats, and visitor log"
      />
      <BoardingStats />
      <Tabs defaultValue="dorms" className="mt-6">
        ...
        <TabsContent value="exeats" className="mt-4">
          <ExeatsTab />
        </TabsContent>
...
function BoardingStats() {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-4">
      ...
      <StatCard
        label="Boarders"
        value={`${data?.boarders ?? 0} / ${data?.capacity ?? 0}`}
        icon={Users}
      />
...
function DormsTab() {
  ...
  const openEdit = (d: any) => {
    ...
    setForm({
      ...
      gender: d.gender,
      capacity: d.capacity,
      warden_name: d.warden_name ?? "",
      warden_phone: d.warden_phone ?? "",
      notes: d.notes ?? "",
    });
    setOpen(true);
  };
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["boarding-summary"] });
    },
      ...
      <CardContent className="p-4">
        <div className="flex justify-between items-center mb-4">
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{editing ? "Edit" : "New"} dormitory</DialogTitle>
              </DialogHeader>
              <div className="grid gap-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Select
                      value={form.gender}
                      onValueChange={(v: DormGender) => setForm({ ...form, gender: v })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setForm({ ...form, warden_name: e.target.value })}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setForm({ ...form, warden_phone: e.target.value })}
                    />
                  </div>
                </div>
                <div>
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground capitalize">
                    {d.gender === "male" ? "Boys" : d.gender === "female" ? "Girls" : "Mixed"} ·
                    Warden: {d.warden_name ?? "—"}
                  </div>
                </div>
              ...
              {results.map((r: any) => (
                <button
                  ...
                  onClick={() => {
                    onChange({
                      id: r.id,
                      label: `${r.full_name} · ${r.matricule ?? ""} · ${r.class_name ?? ""}`,
                    });
                    setQ("");
                  }}
                >
...
function AssignmentsTab() {
  ...
  const [filter, setFilter] = useState<{ dormitoryId: string; q: string }>({
    dormitoryId: "",
    q: "",
  });
  ...
  const create = useMutation({
    mutationFn: () =>
      assignFn({
        data: { student_id: student!.id, dormitory_id: dormId, bed_number: bed || null },
      }),
    onSuccess: () => {
        ...
        <div className="flex flex-wrap justify-between gap-3 mb-4">
          <div className="flex gap-2">
            ...
            <Select
              ...
              onValueChange={(v) => setFilter({ ...filter, dormitoryId: v === "all" ? "" : v })}
            >
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Assign to dormitory</DialogTitle>
              </DialogHeader>
              <div className="grid gap-3">
                ...
                <div>
                  ...
                  <Select value={dormId} onValueChange={setDormId}>
                    <SelectTrigger>
                      <SelectValue placeholder="Pick a dormitory" />
                    </SelectTrigger>
                    <SelectContent>
                      {dorms.map((d: any) => (
                        <SelectItem key={d.id} value={d.id}>
                          {d.name} ({d.occupied}/{d.capacity})
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
              ...
              <DialogFooter>
                <Button
                  ...
                  disabled={!student || !dormId || create.isPending}
                >
                  Assign
                </Button>
              </DialogFooter>
          ...
          <table className="w-full text-sm">
            <thead className="text-xs text-muted-foreground border-b">
              <tr>
                ...
                <th className="text-left py-2 px-2">Dormitory</th>
                <th className="text-left py-2 px-2">Bed</th>
              ...
              {rows.map((r: any) => (
                <tr key={r.id} className="border-b">
                  <td className="py-2 px-2 font-medium">
                    ...
                    <span className="text-xs text-muted-foreground">{r.students?.matricule}</span>
                  </td>
                  ...
                  <td className="py-2 px-2 text-right">
                    <Button size="sm" variant="ghost" onClick={() => release.mutate(r.id)}>
                      Release
                    </Button>
                  </td>
                </tr>
    ...
    mutationFn: () =>
      saveFn({
        data: {
          ...
          date,
          session,
          entries: Object.entries(entries).map(([student_id, v]) => ({
            student_id,
            status: v.status,
            note: v.note || null,
          })),
      ...
      <CardContent className="p-4">
        <div className="flex flex-wrap gap-3 mb-4 items-end">
          <div>
            ...
            <Select value={dormId} onValueChange={setDormId}>
              <SelectTrigger className="w-52">
                <SelectValue placeholder="Pick dormitory" />
              </SelectTrigger>
              <SelectContent>
                {dorms.map((d: any) => (
                  <SelectItem key={d.id} value={d.id}>
            ...
            <table className="w-full text-sm">
              <thead className="text-xs text-muted-foreground border-b">
                <tr>
                  <th className="text-left py-2 px-2">Student</th>
                  <th className="text-left py-2 px-2">Class</th>
                  <th className="text-left py-2 px-2">Room/Bed</th>
              ...
              <tbody>
                  ...
                  return (
                    <tr key={r.student_id} className="border-b">
                      ...
                      <td className="py-2 px-2 text-muted-foreground">
                        {r.dorm_rooms?.room_number ?? "—"} / {r.bed_number ?? "—"}
                      </td>
                      <td className="py-2 px-2">
                        <Select
                          value={e.status}
                          onValueChange={(v: RollStatus) =>
                            setEntries({ ...entries, [r.student_id]: { ...e, status: v } })
                          }
                        >
                      ...
                      <td className="py-2 px-2">
                        <Input
                          ...
                          onChange={(ev) =>
                            setEntries({
                              ...entries,
                              [r.student_id]: { ...e, note: ev.target.value },
                            })
                          }
                          className="h-8"
                        />
                ...
                {(rollQ.data.roster as any[]).length === 0 && (
                  <tr>
                    <td colSpan={5} className="text-center py-8 text-muted-foreground">
                      No boarders assigned to this dormitory.
                    </td>
                  </tr>
                )}
              </tbody>
...
function ExeatsTab() {
  ...
  const [filter, setFilter] = useState<{ status: ExeatStatus | "all"; q: string }>({
    status: "all",
    q: "",
  });
  const { data: rows = [] } = useQuery({
    queryKey: ["exeats", filter],
  ...
  const [form, setForm] = useState({
    reason: "",
    destination: "",
    depart_at: toLocal(now),
    return_by: toLocal(new Date(now.getTime() + 24 * 3600 * 1000)),
    guardian_name: "",
    guardian_phone: "",
    guardian_approved: false,
    guardian_approval_note: "",
    notes: "",
  });
    ...
    mutationFn: () =>
      createFn({
        data: {
          ...
          return_by: new Date(form.return_by).toISOString(),
          guardian_name: form.guardian_name || null,
          guardian_phone: form.guardian_phone || null,
          guardian_approved: form.guardian_approved,
          guardian_approval_note: form.guardian_approval_note || null,
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="max-w-2xl">
              ...
              <div className="grid gap-3">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Medical / Family / Weekend..."
                  />
                </div>
                <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setForm({ ...form, return_by: e.target.value })}
                    />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                ...
                <div className="flex items-start gap-2 border rounded-md p-3 bg-muted/40">
                  <Checkbox
                    id="gappr"
                    checked={form.guardian_approved}
                    onCheckedChange={(v) => setForm({ ...form, guardian_approved: !!v })}
                  />
                  <div className="flex-1">
                    <label htmlFor="gappr" className="text-sm font-medium flex items-center gap-1">
                      <ShieldCheck className="h-4 w-4" /> Guardian approved
                    </label>
                    <Input
                      placeholder="Approval note (phone call, message, in person)..."
                      className="mt-2"
                      value={form.guardian_approval_note}
                      onChange={(e) => setForm({ ...form, guardian_approval_note: e.target.value })}
                      disabled={!form.guardian_approved}
                    />
                  </div>
                </div>
                <div>
              ...
              <DialogFooter>
                <Button
                  onClick={() => create.mutate()}
                  disabled={!student || !form.reason || create.isPending}
                >
                  Submit
                </Button>
              </DialogFooter>
          ...
          <table className="w-full text-sm">
            <thead className="text-xs text-muted-foreground border-b">
              <tr>
                ...
                <th className="text-left py-2 px-2">Depart</th>
                <th className="text-left py-2 px-2">Return by</th>
            ...
            <tbody>
              {rows.map((r: any) => (
                <tr key={r.id} className="border-b">
                  <td className="py-2 px-2 font-medium">
                    {r.students?.full_name}{" "}
                    <span className="text-xs text-muted-foreground">{r.students?.class_name}</span>
                  </td>
                  <td className="py-2 px-2">
                    {r.reason}
                  ...
                  <td className="py-2 px-2 text-right">
                    <Select
                      value=""
                      onValueChange={(v: ExeatStatus) => setStatus.mutate({ id: r.id, status: v })}
                    >
                      <SelectTrigger className="w-32 h-8">
...
function VisitorsTab() {
  ...
  const [filter, setFilter] = useState<{ q: string; activeOnly: boolean }>({
    q: "",
    activeOnly: false,
  });
  const { data: rows = [] } = useQuery({
          ...
          <div className="flex gap-2 items-center">
            ...
            <label className="flex items-center gap-2 text-sm">
              <Checkbox
                ...
                onCheckedChange={(v) => setFilter({ ...filter, activeOnly: !!v })}
              />
              On site only
            </label>
          </div>
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              ...
              <DialogFooter>
                <Button
                  onClick={() => create.mutate()}
                  disabled={!form.visitor_name || create.isPending}
                >
                  Check in
                </Button>
              </DialogFooter>
            ...
            <tbody>
              {rows.map((r: any) => (
                <tr key={r.id} className="border-b">
                  <td className="py-2 px-2 font-medium">
                    {r.visitor_name}
                    <div className="text-xs text-muted-foreground">{r.visitor_phone}</div>
                  </td>
                  <td className="py-2 px-2">
                    {r.students?.full_name ?? "—"}
                    <div className="text-xs text-muted-foreground">{r.students?.class_name}</div>
                  </td>
                  ...
                  <td className="py-2 px-2 text-xs">
                    {r.check_out_at ? (
                      new Date(r.check_out_at).toLocaleString()
                    ) : (
