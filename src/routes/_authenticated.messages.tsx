...
import { listMessages, sendMessage, deleteMessage, type MessageAudience } from "@/lib/messages.functions";
...
function MessagesPage() {
  ...
  const sendMut = useMutation({
    mutationFn: () => send({ data: { audience, audience_class: audClass || undefined, subject, body } }),
    onSuccess: () => {
      ...
      <div className="grid gap-6 lg:grid-cols-[420px_1fr]">
        <Card>
          <CardContent className="space-y-3 p-5">
            ...
            <div className="grid gap-1.5">
              <Label>Subject</Label>
              <Input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="e.g. PTA meeting Friday" />
            </div>
            <div className="grid gap-1.5">
              <Label>Message</Label>
              <Textarea rows={6} value={body} onChange={(e) => setBody(e.target.value)} placeholder="Write your announcement…" />
            </div>
            <Button
              className="w-full"
              onClick={() => sendMut.mutate()}
              disabled={!subject.trim() || !body.trim() || sendMut.isPending || (audience === "class" && !audClass)}
            >
                ...
                <CardContent className="space-y-2 p-4 pl-5">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button size="icon" variant="ghost" onClick={() => delMut.mutate(m.id)} aria-label="Delete">
