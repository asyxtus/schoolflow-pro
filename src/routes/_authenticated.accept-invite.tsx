  ...
  return (
    <div className="min-h-[80vh] grid place-items-center p-6">
      <Card className="w-full max-w-md">
        <CardHeader><CardTitle>Join {school?.name ?? "the school"}</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <div className="text-sm text-muted-foreground">
            You've been invited to join <strong className="text-foreground">{school?.name}</strong>
...
function Center({ children }: { children: React.ReactNode }) {
  return <div className="min-h-[60vh] grid place-items-center text-muted-foreground">{children}</div>;
}