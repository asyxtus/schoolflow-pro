...
export const listCoefficients = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: cls } = await supabase
      ...
      .maybeSingle();
    if (!cls) return { coefficients: [] };
    const { data: rows, error } = await supabase
      .from("class_subjects")
      .select("id, subject, coefficient, staff(first_name, last_name)")
      .eq("school_id", schoolId)
      .eq("class_id", cls.id)
      .order("subject");
    if (error) throw new Error(error.message);
    return {
      coefficients: (rows ?? []).map((r) => {
        const staffRow = (r as { staff?: { first_name?: string; last_name?: string } | null })
          .staff;
        const teacher_name = staffRow
          ? `${staffRow.first_name ?? ""} ${staffRow.last_name ?? ""}`.trim() || null
          : null;
        return {
          id: r.id as string,
...
export const computeBulletin = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    (coefs ?? []).forEach((c) => {
      const staffRow = (c as { staff?: { first_name?: string; last_name?: string } | null }).staff;
      const teacher_name = staffRow
        ? `${staffRow.first_name ?? ""} ${staffRow.last_name ?? ""}`.trim() || null
        : null;
      coefMap.set(c.subject as string, {
    ...
    const perStudent = new Map<
      string,
      { subjects: Map<string, number>; totalWeighted: number; totalCoef: number }
    >();
    (allGrades ?? []).forEach((g) => {
    ...
    subjectSet.forEach((subj) => {
      ...
      const rank =
        mark !== null ? rankArr.findIndex((r) => r.studentId === data.studentId) + 1 || null : null;
    ...
    const classAvgOverall =
      classSize > 0 ? overallArr.reduce((s, r) => s + r.avg, 0) / classSize : null;

    const { data: meta } = await supabase
      .from("bulletin_meta")
      .select(
        "conduct, absences_justified, absences_unjustified, head_teacher_remark, principal_remark",
      )
      .eq("student_id", data.studentId)
      .eq("sequence", data.sequence)
      .maybeSingle();
