...
export const listBooks = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (!schoolId) return [];
    const { data: rows, error } = await context.supabase.from("library_books")
      .select("*").eq("school_id", schoolId).order("title");
    if (error) throw new Error(error.message);
    const term = (data.q ?? "").trim().toLowerCase();
    if (!term) return rows ?? [];
    return (rows ?? []).filter((r) => `${r.title} ${r.author ?? ""} ${r.isbn ?? ""} ${r.category ?? ""}`.toLowerCase().includes(term));
  });
...
export const listCopies = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    const schoolId = await getUserSchoolId(context.supabase, context.userId);
    if (!schoolId) return [];
    const { data: rows, error } = await context.supabase.from("library_copies")
      .select("*").eq("school_id", schoolId).eq("book_id", data.bookId).order("created_at");
    if (error) throw new Error(error.message);
    return rows ?? [];
  });
...
export const addCopies = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const rows = Array.from({ length: count }, (_, i) => ({
      school_id: schoolId, book_id: data.book_id,
...
export const listLoans = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    const schoolId = await getUserSchoolId(context.supabase, context.userId);
    if (!schoolId) return [];
    let q = context.supabase.from("library_loans")
      .select("*, library_books(title, author), library_copies(barcode), students(first_name,last_name,matricule,class_name), staff(first_name,last_name)")
      .eq("school_id", schoolId).order("loaned_at", { ascending: false }).limit(300);
    ...
    return (rows ?? []).map((r) => {
      ...
      const borrower_name = r.borrower_type === "student"
...
export const createLoan = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { copy_id: string; borrower_type: BorrowerType; student_id?: string; staff_id?: string; due_date: string; note?: string }) => d)
  .handler(async ({ context, data }) => {
    const schoolId = await getUserSchoolId(context.supabase, context.userId);
    if (!schoolId) throw new Error("No school");
    const { data: copy, error: ce } = await context.supabase.from("library_copies")
      .select("id, book_id, status").eq("id", data.copy_id).single();
    ...
    const { error: le } = await context.supabase.from("library_loans").insert({
      school_id: schoolId, copy_id: data.copy_id, book_id: copy.book_id,
      borrower_type: data.borrower_type,
      student_id: data.borrower_type === "student" ? data.student_id : null,
      staff_id: data.borrower_type === "staff" ? data.staff_id : null,
      due_date: data.due_date, note: data.note ?? null, recorded_by: context.userId,
    });
    if (le) throw new Error(le.message);
    await context.supabase.from("library_copies").update({ status: "loaned" }).eq("id", data.copy_id);
    return { ok: true };
  });
...
export const returnLoan = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const { data: loan, error } = await context.supabase.from("library_loans")
      .select("id, copy_id, status").eq("id", data.id).single();
    ...
    await context.supabase.from("library_loans").update({ status: newStatus, returned_at: new Date().toISOString() }).eq("id", loan.id);
    await context.supabase.from("library_copies").update({ status: copyStatus }).eq("id", loan.copy_id);
    return { ok: true };
  });
...
export const listReservations = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const schoolId = await getUserSchoolId(context.supabase, context.userId);
    if (!schoolId) return [];
    const { data, error } = await context.supabase.from("library_reservations")
      .select("*, library_books(title, author, available_copies), students(first_name,last_name,matricule,class_name), staff(first_name,last_name)")
...
export const createReservation = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { book_id: string; borrower_type: BorrowerType; student_id?: string; staff_id?: string; note?: string }) => d)
  .handler(async ({ context, data }) => {
    const schoolId = await getUserSchoolId(context.supabase, context.userId);
    if (!schoolId) throw new Error("No school");
    const { error } = await context.supabase.from("library_reservations").insert({
      school_id: schoolId, book_id: data.book_id, borrower_type: data.borrower_type,
...
export const searchLibraryStudents = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: rows, error } = await context.supabase.from("students")
      .select("id, first_name, last_name, matricule, class_name").eq("school_id", schoolId)
      .or(`first_name.ilike.%${term}%,last_name.ilike.%${term}%,matricule.ilike.%${term}%`).limit(20);
    if (error) throw new Error(error.message);
...
export const listStaffLite = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const schoolId = await getUserSchoolId(context.supabase, context.userId);
    if (!schoolId) return [];
    const { data, error } = await context.supabase.from("staff")
      .select("id, first_name, last_name, position").eq("school_id", schoolId).eq("status", "active").order("last_name");
    if (error) throw new Error(error.message);
...
export const librarySummary = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const schoolId = await getUserSchoolId(context.supabase, context.userId);
    if (!schoolId) return { titles: 0, copies: 0, available: 0, activeLoans: 0, overdue: 0, reservations: 0 };
    const today = new Date().toISOString().slice(0, 10);
    const [b, c, l, o, r] = await Promise.all([
      context.supabase.from("library_books").select("id", { count: "exact", head: true }).eq("school_id", schoolId),
      context.supabase.from("library_copies").select("id, status", { count: "exact" }).eq("school_id", schoolId),
      context.supabase.from("library_loans").select("id", { count: "exact", head: true }).eq("school_id", schoolId).eq("status", "active"),
      context.supabase.from("library_loans").select("id", { count: "exact", head: true }).eq("school_id", schoolId).eq("status", "active").lt("due_date", today),
      context.supabase.from("library_reservations").select("id", { count: "exact", head: true }).eq("school_id", schoolId).eq("status", "pending"),
    ]);
    const copies = c.count ?? 0;
    const available = (c.data ?? []).filter((x) => x.status === "available").length;
    return { titles: b.count ?? 0, copies, available, activeLoans: l.count ?? 0, overdue: o.count ?? 0, reservations: r.count ?? 0 };
  });