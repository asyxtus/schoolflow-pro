...
export const upsertVendor = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (d: { id?: string; name: string; phone?: string; email?: string; note?: string }) => d,
  )
  .handler(async ({ context, data }) => {
    const { supabase, userId } = context;
    const schoolId = await getUserSchoolId(supabase, userId);
...
export const cashPosition = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const [pays, exps, runs, pending] = await Promise.all([
      supabase
        .from("payments")
        .select("amount_fcfa, method, paid_at")
        .eq("voided", false)
        .eq("school_id", schoolId)
        .gte("paid_at", from)
        .lte("paid_at", to),
      supabase
        .from("expenses")
        .select("amount_fcfa, method, spent_at, status, expense_categories(name)")
        .eq("school_id", schoolId)
        .eq("status", "approved")
        .gte("spent_at", from)
        .lte("spent_at", to),
      supabase
        .from("payroll_runs")
        .select("total_net_fcfa, status, period")
        .eq("school_id", schoolId),
      supabase
        .from("expenses")
        .select("amount_fcfa")
    ...
    for (const p of pays.data ?? [])
      inMap.set(p.method, (inMap.get(p.method) ?? 0) + (p.amount_fcfa ?? 0));
    const outMap = new Map<string, number>();
    for (const e of exps.data ?? [])
      outMap.set(e.method, (outMap.get(e.method) ?? 0) + (e.amount_fcfa ?? 0));
    ...
    return {
      totals: {
        ...
        expenses,
        payroll,
        net: collected - expenses - payroll,
        pendingExpenses,
      },
