...
export const createInvitation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (!MANAGEABLE_ROLES.includes(data.role)) throw new Error("That role cannot be assigned via invitation");
...
export const getInvitationByToken = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: inv, error } = await supabase
      .from("staff_invitations")
      .select("id, email, role, status, expires_at, school_id, schools(name, city, region)")
      .eq("token", data.token)
      .maybeSingle();
    if (error) throw new Error(error.message);
    return inv;
...
export const acceptInvitation = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    // Assign school + role using admin client (RLS blocks self-assign of roles)
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    await supabaseAdmin.from("profiles").update({ school_id: inv.school_id }).eq("id", userId);
    await supabaseAdmin
      .from("user_roles")
      .upsert({ user_id: userId, school_id: inv.school_id, role: inv.role }, { onConflict: "user_id,role,school_id" });
    await supabaseAdmin
      .from("staff_invitations")
...
export const listAuditLog = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    let q = supabase
      .from("audit_log")
      .select("id, actor_id, actor_email, action, entity_type, entity_id, summary, before, after, created_at")
      .eq("school_id", schoolId)
      .order("created_at", { ascending: false })
    ...
    return (rows ?? []).filter((r) =>
      `${r.action} ${r.entity_type} ${r.summary ?? ""} ${r.actor_email ?? ""}`.toLowerCase().includes(search),
    );
  });