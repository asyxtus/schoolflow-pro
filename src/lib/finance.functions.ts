...
export const upsertFeeStructure = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    // Enforce single tuition structure per class *per academic year* (allow
    // editing the existing one). Previously this ignored academic_year
    // entirely, which meant a school could never create next year's tuition
    // fee for a class without first overwriting this year's.
    if ((data.kind ?? "tuition") === "tuition") {
      let query = supabase
        ...
        .eq("kind", "tuition");
      query = data.academic_year
        ? query.eq("academic_year", data.academic_year)
        : query.is("academic_year", null);
      const { data: existing } = await query;
      const others = (existing ?? []).filter((r) => r.id !== data.id);
      if (others.length > 0) {
        throw new Error(
          `A tuition fee already exists for ${data.class_name}${data.academic_year ? ` in ${data.academic_year}` : ""}. Edit the existing one instead of creating a new one.`,
        );
      }
    }
    ...
    // Diocese-linked schools need approval before a tuition change takes
    // effect — enforced for real by a database trigger on fee_structures,
    // not just this check, so this only exists to give a clean redirect
    // instead of a raw trigger-rejection error.
    if ((data.kind ?? "tuition") === "tuition") {
      ...
      if (schoolRow?.diocese_id) {
        ...
        if (data.id) {
          const { data: current } = await supabase
            ...
            .eq("id", data.id)
            .maybeSingle();
          amountChanged = current?.amount_fcfa !== total;
        }
        if (amountChanged) {
          const { error: reqErr } = await supabase.from("approval_requests").insert({
            school_id: schoolId,
            request_type: "fee_structure_change",
            title: `Tuition change — ${data.class_name} (${data.label})`,
            description: `Requested new amount: ${total.toLocaleString()} FCFA`,
            amount_fcfa: total,
            payload: {
              class_name: data.class_name,
              label: data.label,
              kind: data.kind ?? "tuition",
              academic_year: data.academic_year ?? null,
              new_amount_fcfa: total,
              fee_structure_id: data.id ?? null,
            },
            requested_by: userId,
          });
...
export const listPayments = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    let q = supabase
      .from("payments")
      .select(
        "id, student_id, amount_fcfa, method, reference, note, paid_at, receipt_no, voided, void_reason, students(first_name,last_name,matricule,class_name)",
      )
      .eq("school_id", schoolId)
    ...
    return (rows ?? []).filter((r) => {
      ...
      const hay =
        `${s?.first_name ?? ""} ${s?.last_name ?? ""} ${s?.matricule ?? ""} ${s?.class_name ?? ""} ${r.reference ?? ""} ${r.receipt_no ?? ""}`.toLowerCase();
...
export const recordPayment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (
      ["momo", "bank", "cheque"].includes(data.method) &&
      !(data.reference && data.reference.trim())
    ) {
      throw new Error(
        "A reference (transaction ID, cheque #, deposit slip) is required for non-cash payments.",
      );
    }
    ...
    if (data.allocations && data.allocations.length > 0) {
      for (const a of data.allocations) {
        ...
        if (bal === undefined)
          throw new Error(
            "An invoice in this payment does not belong to this student or is already settled.",
          );
    ...
    if (plan.length > 0 && inserted) {
      ...
      if (aErr)
        throw new Error(`Payment saved but could not be applied to invoices: ${aErr.message}`);
    }
...
export const voidPayment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (!data.reason || data.reason.trim().length < 4)
      throw new Error("A reason (≥ 4 chars) is required to void a payment.");
    const { data: existing, error: fErr } = await supabase
      .from("payments")
      .select("id, voided, school_id, student_id, amount_fcfa, receipt_no")
      .eq("id", data.id)
      .single();
...
export const closeDay = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const schoolId = await getUserSchoolId(supabase, userId);
    if (!schoolId) throw new Error("No school");
    const { data: canClose } = await supabase.rpc(
      "can_record_payments" as never,
      { _user_id: userId, _school_id: schoolId } as never,
    );
    ...
    const { data: pays } = await supabase
      .from("payments")
      .select("amount_fcfa, method, voided")
      .eq("school_id", schoolId)
      .gte("paid_at", start)
      .lte("paid_at", end);
...
export const financeSummary = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const thisMonth = (pays.data ?? [])
      .filter((r) => r.paid_at >= monthStart)
      .reduce((a, r) => a + (r.amount_fcfa ?? 0), 0);
...
export const listStudentFees = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    let q = supabase
      ...
      .select(
        "id, student_id, fee_structure_id, label, kind, amount_fcfa, discount_fcfa, net_fcfa, paid_fcfa, balance_fcfa, status, academic_year, due_date, note, created_at",
      )
      .eq("school_id", schoolId)
    ...
    const withStudent = (rows ?? []).map((r) => ({
      ...r,
      students: map.get(r.student_id as string) ?? null,
    }));
...
// Void an invoice: only once no payment has ever been applied would a plain
// delete work — once money has touched it, this is the only way to cancel
// it. Requires a reason; bursar/principal only (enforced again in the DB).
export const voidStudentFee = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { id: string; reason: string }) => d)
  .handler(async ({ context, data }) => {
    const { supabase, userId } = context;
    if (!data.reason || data.reason.trim().length < 4)
      throw new Error("A reason (≥ 4 chars) is required to void an invoice.");
    const { data: existing, error: fErr } = await supabase
      .from("student_fees")
      .select("id, voided, school_id, student_id, label, amount_fcfa")
      .eq("id", data.id)
      .single();
    if (fErr) throw new Error(fErr.message);
    if (existing.voided) throw new Error("This invoice is already voided.");
    const { error } = await supabase
      .from("student_fees")
      .update({
        voided: true,
    ...
    await supabase.rpc("log_audit", {
      _school_id: existing.school_id,
      _action: "void_invoice",
      _entity_type: "student_fee",
      _entity_id: data.id,
      _summary: `Voided invoice "${existing.label}" (${existing.amount_fcfa} FCFA) — ${data.reason.trim()}`,
    });
    return { ok: true };
  });
  ...
  .handler(async ({ context, data }) => {
    ...
    if (data.fee_structure_id) {
      const { data: fs } = await supabase
        ...
        .single();
      if (fs) {
        label = label ?? fs.label;
        amount = amount ?? fs.amount_fcfa;
        year = year ?? fs.academic_year ?? undefined;
      }
    }
    ...
    if (!students?.length) return { ok: true, count: 0, skipped: 0 };

    // Don't double-bill: find students in this class who already have a
    // non-voided invoice for this exact fee (by structure if linked, else by
    // label), and only bill the rest. Re-running this by mistake — a second
    // click, a retried request — used to either duplicate every invoice in
    // the class or, if a unique index caught one row, fail the whole batch
    // and silently block legitimate billing for everyone else too.
    let alreadyBilled = new Set<string>();
    ...
    if (data.fee_structure_id) {
      ...
      alreadyBilled = new Set((existing ?? []).map((r) => r.student_id as string));
    } else {
      const { data: existing } = await supabase
        .from("student_fees")
        .select("student_id")
    ...
    if (toBill.length === 0) {
      return { ok: true, count: 0, skipped: students.length };
    }

    const rows = toBill.map((s) => ({
      school_id: schoolId,
      student_id: s.id,
      fee_structure_id: data.fee_structure_id ?? null,
      label: label!,
    ...
    if (error) throw new Error(error.message);
    return { ok: true, count: rows.length, skipped: alreadyBilled.size };
  });
...
export const getPaymentReceipt = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: p, error } = await supabase
      .from("payments")
      .select(
        "id, amount_fcfa, method, reference, note, paid_at, receipt_no, student_id, students(first_name,last_name,matricule,class_name,fee_balance)",
      )
      .eq("id", data.id)
      .eq("school_id", schoolId)
      .single();
    ...
    const { data: school } = await supabase
      .from("schools")
      .select("name, code, city, region, motto")
      .eq("id", schoolId)
      .single();
    ...
    const allocations = (allocs ?? []).map((a) => ({
      ...
      due_date:
        (a as { student_fees?: { due_date?: string | null } }).student_fees?.due_date ?? null,
    }));
