...
function startOf(kind: "day" | "week" | "month"): string {
  const now = new Date();
  if (kind === "day") return new Date(now.getFullYear(), now.getMonth(), now.getDate()).toISOString();
...
export const getSchoolWalletDefaults = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data } = await supabase
      .from("schools")
      .select("wallet_default_per_txn_limit, wallet_default_daily_limit, wallet_default_weekly_limit, wallet_default_monthly_limit")
      .eq("id", schoolId)
      .single();
    return {
...
export const updateStudentWalletLimits = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    const { error } = await context.supabase.from("students").update({
...
export const getStudentWalletContext = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const [{ data: st }, { data: sch }] = await Promise.all([
      supabase.from("students").select("id, first_name, last_name, wallet_balance, wallet_per_txn_limit, wallet_daily_limit, wallet_weekly_limit, wallet_monthly_limit").eq("id", data.student_id).single(),
      supabase.from("schools").select("wallet_default_per_txn_limit, wallet_default_daily_limit, wallet_default_weekly_limit, wallet_default_monthly_limit").eq("id", schoolId).single(),
    ]);
    ...
    const usage = { day: { count: 0, sum: 0 }, week: { count: 0, sum: 0 }, month: { count: 0, sum: 0 } };
    for (const r of recent ?? []) {
      usage.month.count++; usage.month.sum += r.amount_fcfa;
      if (r.occurred_at >= weekStart) { usage.week.count++; usage.week.sum += r.amount_fcfa; }
      if (r.occurred_at >= dayStart) { usage.day.count++; usage.day.sum += r.amount_fcfa; }
    }
...
export const recordWalletTransaction = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { student_id: string; kind: WalletKind; amount_fcfa: number; method: WalletMethod; reference?: string; note?: string; occurred_at?: string; guardian_approved?: boolean; guardian_approval_note?: string }) => d)
  .handler(async ({ context, data }) => {
    const { supabase, userId } = context;
    const schoolId = await getUserSchoolId(supabase, userId);
    if (!schoolId) throw new Error("No school assigned");
    ...
    if (data.kind === "withdrawal") {
      ...
      const [{ data: stLim }, { data: sch }] = await Promise.all([
        supabase.from("students").select("wallet_per_txn_limit, wallet_daily_limit, wallet_weekly_limit, wallet_monthly_limit").eq("id", data.student_id).single(),
        supabase.from("schools").select("wallet_default_per_txn_limit, wallet_default_daily_limit, wallet_default_weekly_limit, wallet_default_monthly_limit").eq("id", schoolId).single(),
      ]);
      const eff = {
      ...
      if (eff.per_txn != null && data.amount_fcfa > eff.per_txn) reasons.push(`exceeds per-transaction limit (${eff.per_txn} FCFA)`);
      if (eff.daily != null || eff.weekly != null || eff.monthly != null) {
        ...
        const day = startOf("day"), week = startOf("week");
        ...
        if (eff.daily != null && dCount + 1 > eff.daily) reasons.push(`exceeds daily frequency (${eff.daily}/day)`);
        if (eff.weekly != null && wCount + 1 > eff.weekly) reasons.push(`exceeds weekly frequency (${eff.weekly}/week)`);
        if (eff.monthly != null && mCount + 1 > eff.monthly) reasons.push(`exceeds monthly frequency (${eff.monthly}/month)`);
      }
      if (reasons.length > 0) {
        overLimit = true;
...
export const listStudentWalletBalances = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    let q = supabase
      .from("students")
      .select("id, first_name, last_name, matricule, class_name, wallet_balance, wallet_per_txn_limit, wallet_daily_limit, wallet_weekly_limit, wallet_monthly_limit")
      .eq("school_id", schoolId)
      .eq("status", "active")
      .order("class_name")
      .order("last_name");
