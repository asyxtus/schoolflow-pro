...
export const listStaffAttendance = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: att } = ids.length
      ? await supabase.from("staff_attendance").select("*").eq("school_id", schoolId).eq("work_date", data.date).in("staff_id", ids)
      : { data: [] as never[] };
...
export const upsertStaffAttendance = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { staff_id: string; work_date: string; status: StaffAttStatus; clock_in?: string | null; clock_out?: string | null; note?: string }) => d)
  .handler(async ({ context, data }) => {
    const { supabase, userId } = context;
    const schoolId = await getUserSchoolId(supabase, userId);
...
export const teacherPerformance = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: grades } = await supabase
      .from("grades")
      .select("student_id, sequence, subject, created_at, ca_score, exam_score, students(class_name)")
      .eq("school_id", schoolId).gte("created_at", `${since}T00:00:00Z`);

    // 5. Own punctuality
    const { data: myAtt } = await supabase
      .from("staff_attendance")
      .select("staff_id, status")
      .eq("school_id", schoolId).gte("work_date", since);
    ...
    for (const t of teachers ?? []) {
      ...
      const takenKeys = new Set(relevantAtt.map((a) => `${(a as { students?: { class_name?: string } }).students?.class_name}|${a.subject}|${a.date}`));
      ...
      for (const g of gradesForTeacher) {
        const ca = Number(g.ca_score ?? 0), ex = Number(g.exam_score ?? 0);
