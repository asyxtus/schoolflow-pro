...
export const listBudgets = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (!schoolId)
      return {
        rows: [] as Array<{ category_id: string; name: string; budget: number; spent: number }>,
      };
    ...
    const rows = (cats.data ?? []).map((c) => {
      const budget = (budgets.data ?? []).find(
        (b) =>
          b.category_id === c.id &&
          (data.month ? b.period_month === data.month : b.period_month === null),
      );
      return {
        category_id: c.id,
        name: c.name,
...
export const upsertBudget = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator(
    (d: { category_id: string; year: number; month: number | null; amount_fcfa: number }) => d,
  )
  .handler(async ({ context, data }) => {
    const { supabase, userId } = context;
    const schoolId = await getUserSchoolId(supabase, userId);
