...
export const getOrCreatePortalToken = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (existing.data && !data.rotate) return { token: existing.data.token, active: existing.data.active };
    ...
    if (existing.data) {
      ...
      const { error } = await supabase.from("student_portal_tokens").insert({
        school_id: schoolId, student_id: data.studentId, token, created_by: userId,
      });
      if (error) throw new Error(error.message);
    }
