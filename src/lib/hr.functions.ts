...
export type StaffPosition =
  | "teacher" | "principal" | "vice_principal" | "bursar" | "secretary"
  | "discipline_master" | "librarian" | "nurse" | "driver" | "cook"
  | "cleaner" | "security" | "maintenance" | "other";
...
export const upsertStaff = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (data.id) {
      const { error } = await supabase.from("staff").update(payload).eq("id", data.id).eq("school_id", schoolId);
      if (error) throw new Error(error.message);
      return { id: data.id };
    }
...
export const deleteStaff = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const schoolId = await getUserSchoolId(supabase, userId);
    if (!schoolId) throw new Error("No school assigned");
    const { error } = await supabase.from("staff").delete().eq("id", data.id).eq("school_id", schoolId);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const upsertAllowance = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { id?: string; staff_id: string; label: string; kind: "allowance" | "deduction"; amount_fcfa: number; active?: boolean }) => d)
  .handler(async ({ context, data }) => {
    const { supabase, userId } = context;
    const schoolId = await getUserSchoolId(supabase, userId);
    if (!schoolId) throw new Error("No school assigned");
    ...
    const { data: row, error } = await supabase.from("staff_allowances").insert(payload).select("id").single();
    if (error) throw new Error(error.message);
    return { id: row.id };
  });
...
export const createPayrollRun = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: run, error } = await supabase
      .from("payroll_runs")
      .insert({ school_id: schoolId, period: data.period, notes: data.notes ?? null, created_by: userId })
      .select("id")
      .single();
    if (error) throw new Error(error.message);
    ...
    const [{ data: staff }, { data: allowances }] = await Promise.all([
      supabase.from("staff").select("id, base_salary_fcfa").eq("school_id", schoolId).eq("status", "active"),
      supabase.from("staff_allowances").select("staff_id, label, kind, amount_fcfa, active").eq("school_id", schoolId).eq("active", true),
    ]);
    const byStaff = new Map<string, { allowances: { label: string; amount: number }[]; deductions: { label: string; amount: number }[] }>();
...
export const getPayrollRun = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const [run, slips] = await Promise.all([
      supabase.from("payroll_runs").select("*").eq("id", data.id).eq("school_id", schoolId).maybeSingle(),
      supabase
        .from("payslips")
        .select("*, staff:staff_id(id, first_name, last_name, matricule, position, bank_account, momo_number)")
        .eq("run_id", data.id)
        .order("created_at"),
    ]);
...
export const deletePayrollRun = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const schoolId = await getUserSchoolId(supabase, userId);
    if (!schoolId) throw new Error("No school assigned");
    const { error } = await supabase.from("payroll_runs").delete().eq("id", data.id).eq("school_id", schoolId);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const markPayslipPaid = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { id: string; payment_method: PayMethod; reference?: string | null; paid_at?: string | null }) => d)
  .handler(async ({ context, data }) => {
    const { supabase } = context;
