...
export type ExeatStatus =
  "pending" | "approved" | "denied" | "departed" | "returned" | "overdue" | "cancelled";
  ...
  .handler(async ({ context }) => {
    ...
    if (ids.length) {
      ...
      for (const a of assigns ?? [])
        occupancy[a.dormitory_id] = (occupancy[a.dormitory_id] ?? 0) + 1;
    }
...
export const listBoardingAssignments = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    let q = supabase
      .from("boarding_assignments")
      .select(
        "id, student_id, dormitory_id, room_id, bed_number, assigned_on, active, students(first_name, last_name, matricule, class_name, gender), dormitories(name), dorm_rooms(room_number)",
      )
      .eq("school_id", schoolId)
      .eq("active", true)
    ...
    let out = (rows ?? []).map((r: any) => ({
      ...
      students: r.students
        ? {
            ...r.students,
            full_name: `${r.students.first_name ?? ""} ${r.students.last_name ?? ""}`.trim(),
          }
        : r.students,
    }));
    if (data.q) {
      ...
      out = out.filter(
        (r: any) =>
          r.students?.full_name?.toLowerCase().includes(t) ||
          r.students?.matricule?.toLowerCase().includes(t),
      );
    }
    return out;
  });
...
export const assignBoardingStudent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    await supabase
      .from("boarding_assignments")
      .update({ active: false, released_on: new Date().toISOString().slice(0, 10) })
...
export const getRollCall = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: assigns } = await supabase
      .from("boarding_assignments")
      .select(
        "student_id, bed_number, dorm_rooms(room_number), students(first_name, last_name, matricule, class_name)",
      )
      .eq("school_id", schoolId)
      .eq("dormitory_id", data.dormitory_id)
      .eq("active", true);
    const { data: marks } = await supabase
      ...
      .eq("school_id", schoolId)
      .eq("dormitory_id", data.dormitory_id)
      .eq("roll_date", data.date)
      .eq("session", data.session);
    const map: Record<string, { status: RollStatus; note: string | null }> = {};
    for (const m of marks ?? [])
      map[m.student_id] = { status: m.status as RollStatus, note: m.note };
    const roster = (assigns ?? []).map((r: any) => ({
      ...r,
      students: r.students
        ? {
            ...r.students,
...
export const saveRollCall = createServerFn({ method: "POST" })
  ...
  .inputValidator(
    (d: {
      ...
      date: string;
      session: RollSession;
      entries: Array<{ student_id: string; status: RollStatus; note?: string | null }>;
    }) => d,
  )
  .handler(async ({ context, data }) => {
    const { supabase, userId } = context;
...
export const listExeats = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    for (const r of out as any[]) {
      if (
        (r.status === "departed" || r.status === "approved") &&
        new Date(r.return_by).getTime() < now &&
        !r.actual_return_at
      ) {
        r.status = "overdue";
      }
...
export const createExeat = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { error } = await supabase.from("boarding_exeats").insert({
      ...
      guardian_approval_note: data.guardian_approval_note ?? null,
      notes: data.notes ?? null,
      status: data.guardian_approved ? "approved" : "pending",
      approved_by: data.guardian_approved ? userId : null,
      approved_at: data.guardian_approved ? new Date().toISOString() : null,
      created_by: userId,
    });
    if (error) throw new Error(error.message);
    return { ok: true };
...
export const listVisitors = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    if (data.q) {
      ...
      out = out.filter(
        (r: any) =>
          r.visitor_name?.toLowerCase().includes(t) ||
          r.students?.full_name?.toLowerCase().includes(t),
      );
    }
    return out;
  ...
  .handler(async ({ context }) => {
    ...
    const [dorms, boarders, exeats, visitors] = await Promise.all([
      ...
      supabase
        ...
        .select("id", { count: "exact", head: true })
        .eq("school_id", schoolId)
        .in("status", ["approved", "departed", "overdue"]),
      supabase
        .from("boarding_visitors")
        .select("id", { count: "exact", head: true })
        .eq("school_id", schoolId)
