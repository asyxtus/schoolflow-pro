...
export const financeReport = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const [pays, fees, studs, wallet, runs, subs, incidents] = await Promise.all([
      ...
      supabase
        .from("payroll_runs")
        .select("total_net_fcfa, period, status")
        .eq("school_id", schoolId),
      supabase
    ...
    const invoiced = (fees.data ?? []).reduce(
      (a, r) => a + Math.max((r.amount_fcfa ?? 0) - (r.discount_fcfa ?? 0), 0),
      0,
    );
    ...
    for (const p of pays.data ?? [])
      methodMap.set(p.method, (methodMap.get(p.method) ?? 0) + (p.amount_fcfa ?? 0));
