...
export const getDashboardStats = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [
      ...
      supabase.from("applicants").select("id, stage", { count: "exact" }).eq("school_id", schoolId),
      supabase
        .from("payments")
      ...
      supabase
        .from("payments")
        .select(
          "id, amount_fcfa, method, paid_at, receipt_no, students(first_name,last_name,class_name)",
        )
        .eq("school_id", schoolId)
        .eq("voided", false)
    ...
    const totalBilledAll = [...classAgg.values()].reduce((s, v) => s + v.billed, 0);
    const feeCollectionRate = totalBilledAll
      ? Math.round((Math.max(totalBilledAll - totalBalance, 0) / totalBilledAll) * 100)
      : 0;
    ...
    const recent = (recentPays.data ?? []).map((p) => ({
      ...
      student_name:
        `${(p as { students?: { first_name?: string } }).students?.first_name ?? ""} ${(p as { students?: { last_name?: string } }).students?.last_name ?? ""}`.trim(),
